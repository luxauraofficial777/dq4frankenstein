...
**Compiled:** 2026-09-13 (updated 2026-09-16: added `YAMANA_HBE_HBD_DMA_SUBBLOC_ROUTINE.md` and the 10-doc `HBE-PSX-ENG-SPEC-2026-V2…V11` suite; updated 2026-09-18: added `NAKAMURA_YAMANA_FORMAL_SYSTEMS_DEEP_STUDY.md` and `YAMANA_HBE_16BIT_SYMBOL_ARCHITECTURE_AND_POINTER_PARITY.md`) · **License:** CC BY-NC-SA 4.0
...
| `cybergrimev1.1.zip` (+ `.z01`) | ~7.8 MB | **CyberGrime v1.1 release bundle** (Release Document **v1.1 — August 12, 2026**, Lux Aura / Antigravity Suite) — the incremental CP0/interface upgrade built on the V0.1-era harness (emulator `psx_agent_runner`/`psx_headless`/`psx_test_station`/`hypervisor_core`, `agent_harness.cpp/h`, RAM miner, `audit_output\control_code_bible.json` + `hbd_block_catalog.md`). V1.1 delta: **CP0 coprocessor register mapping** — 32-byte `CyberGrimeCP0Map` exporting Status/Cause/EPC/BadVAddr/Count/Compare/EntryHi/Index, bound to GammaLanguage v1.11C opcode `CP0_REG_MAP` (`0xA3`), verified against `A2AST.h`; **thread-safe signal handlers** (zero collisions under 1,000,000-frame stress test); `@INVARIANT` preconditions verifying CP0 state before kernel-mode ops. Two-part split (`.z01` 5 MB + `.zip` ~2.8 MB), 565 entries, reassemble in order. |
...
| `YAMANA_HBE_16BIT_SYMBOL_ARCHITECTURE_AND_POINTER_PARITY.md` | 7.2 KB / 128 lines | **Forensic study (2026-09-18)** — "HeartBeat 2001 16-Bit Symbol Architecture & Dialogue Pointer Bitstream Parity" (Author: Google DeepMind Agentic Assistant / Antigravity; Baseline: Manabu Yamana 16-bit leaf symbol model, Tokyo 2001) — the **16-bit leaf partition** (Latin/punct `0x0140–0x029A`, Shift-JIS high-bit-stripped kana/kanji `((b0&0x7F)<<8)|b1` with `0x8000` re-OR at VM decoder `0x8008F59C`, control tokens `0x7F01–0x7F4C`/`0x0000`, bit-15-clear leaf rule); root-cause of typographic/pointer desync regressions (missing atlas glyphs bloat the HTS tree; `parse_dialog_pointers` inspected `offset+end` instead of `offset+end+4` → all 135 pointer tables / 1,413 pointers rejected; packed `(BlockID≪20)|BitOffset` compared unmasked); `char_to_yamana_leaf` four-tier ingestion (normalization `“”’—…`→ASCII, accent strip, Shift-JIS fallback, zero-bloat fail-safe → +0 tree symbols); 1:1 bitstream-to-pointer parity rewrite (`dp_bytes=4+8n`, end-marker@`new_end`, count@`+4`, pointers@`+8`); Step −0.5 `sanitize_corpus.py` wired into `shipC/tools/native_build_shipC.py`. |
...
| `NAKAMURA_YAMANA_FORMAL_SYSTEMS_DEEP_STUDY.md` | **Formal systems deep-study (Doc ID `VW-DEEP-WP-001`, Rev A, 2026-09-18)** — "FROM 65816 TO MIPS: The Architecture and Technical Lineage of Nakamura & Yamana's JRPG Engines" — Enix Engine Specification Suite companion covering DQ1–VII on the Famicom/NES (1986–1992), SFC (1992–1996), and PS1 (2000–2001). Five mandated topics, tier-labelled PRESS/HARDWARE/MEASURED/DECODED/INFERRED: (1) **host workstations & toolchains** — the computational autobiography (PC-8001→8801→9801 cross-assembly, BASIC→assembly, no printer, no ICE on any target, line-by-line debugging by eye; PS1 PsyQ-era link-surface `psyq 11/13` pass); (2) **proprietary scripting VMs / zero-copy execution** — FC "addresses-as-data" G1, 65816 inline-operand dispatch with return-address rewriting (`JSL` `$C4:28FE`, `BRK #$XXXX` = message-ID pointer), PS1 byte-aligned `C0 21 A0 <offset:2><dialogId:2>` VM + `%A/%B/%H` conditionals + message-window lifecycle; (3) **memory pagination & bank-switching** — MMC1 5-bit bank protocol, LoROM vs HiROM windowed 24-bit bit-addressed tables, Kseg0/1 + scratchpad + DMA ch3 CD sector-chain banking; (4) **compression** — the G1→G4 Huffman bloodline (per-block bit-order hazards, DQ3 MSB vs DQ6 LSB, adaptive `$7F0000` RAM dictionary, LZSS→LZSSO 4,096-B ring, font tile caching); (5) **65816→MIPS lineage** — five generations / five mechanisms, surviving patterns (dual-base trees, AI bit-fields, inline-operand dispatch, streaming-as-memory), and what the G5 DS sidestep proves; restoration implications for the Sovereign + honest gaps appendix; full artifact index. |
