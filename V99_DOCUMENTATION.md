...
**Version**: V.99 (Pre-release / Release Candidate)
**Date**: August 10, 2026
**Author**: VoidWalkers Project
...
1. Take the DQ4 Japan disc as the base (351 MB, Mode 2 Form 1)
2. Swap in the DW7 US executable (SLUSP012.06_clean.bin)
3. Apply 127+ binary patches to the EXE:
...
4. Re-encode the HBD at sector 362 with English text using the hybrid Huffman tree
5. Run EDC/ECC repair and verification on the output disc
6. Verify all 19 hard gates pass
...
1. Reads DQ4 HBD blocks from the source disc at sector 362
2. For each block: decode using per-block Huffman tree
...
6. Blocks that fail re-encoding get verbatim fallback (DW7 format headers, original text)
